...
/*生成长度为L的随机大整数*/
BigInt randBigint(unsigned int L){
	BigInt R;
	char ic[] = "0123456789";
	srand(int(time(0)));
	for(int i=0; i<L; i++){
	...
	R.values.erase(0, R.values.find_first_not_of('0'));
	return R;
}


namespace Nrsa{
	BigInt p("687840301");////11541388564223956205497074062654910152724228223，大素数电脑带不动，小数测试
	BigInt q("2305843009213693951");//415924872698612520126277439911
	BigInt a("1"),b("2");
	BigInt N = p*q;
	BigInt s=gcd(p-a,q-a);
	BigInt n=s*((p-a)/s)*((q-a)/s);
	
}

/*函数：生成公私钥对*/
bool genePrPu(BigInt& e,BigInt& d){ 
	using namespace Nrsa;
	for(int L=Nrsa::N.values.length()/2; L<=Nrsa::n.values.length(); ++L){
		e = randBigint(L);
		if(gcd(e,Nrsa::n).values=="1"&&(Nrsa::n-e).flag==true){
			d = mod_inverse(e,n);
			return true;
		}	
		else e.values = "0";
...
/*RSA加密*/
BigInt rsaEncrypt(BigInt m,BigInt e)
{
	if(e.values.length() < m.values.length())//实际上m长度要小于n
		return m;
	else return mod_fast(m,e,Nrsa::N);
    
}
BigInt rsaDncrypt(BigInt m,BigInt d)
{
	if(d.values.length() < m.values.length())
		return m;
    else return mod_fast(m,d,Nrsa::N);
}
#endif 
